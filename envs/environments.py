...
class Rectangle(Environment):
    ...
    def relation(self, s1, s2):  # 根据state计算真实应该行动的方向
    ...
    def state_data(self):  # 确定每个位置对应的sense
...
class Hexagonal(Environment):
    def __init__(self, params, width):
        self.n_states, self.graph_states = self.get_n_states(width)
        super().__init__(params, width, width)

    @staticmethod
    def get_n_states(width):
        return int((3 * (width ** 2) + 1) / 4), width ** 2
...
class FamilyTree(Environment):
    def __init__(self, params, levels):
        self.levels = levels
        self.n_states = self.get_n_states(levels)
        super().__init__(params, levels, levels)

    @staticmethod
    def get_n_states(levels):
        return 2 ** (levels + 1) - 1
...
class LineTI(Environment):
    def __init__(self, params, length):
        ...
        self.n_states = self.get_n_states(length)
        super().__init__(params, length, length)

    @staticmethod
    def get_n_states(length):
        return length
...
class Wood2000(Environment):
    ...
    @staticmethod
    def get_n_states(width, height):
        return 4 * (height - 1 + width - 1) + 2 * (width - 1)
...
class Frank2000(Environment):
    def __init__(self, params, width, height):
        self.n_states, self.n_states_base = self.get_n_states(width, height)
        super().__init__(params, width, height)
    ...
    def get_n_states(width, height):
        ...
        return n_states_base + 2 * (width + height), n_states_base
    ...
    def world(self):
        ...
        width = self.width
        height = self.height
        n_states_base = self.n_states_base
        n_states = self.n_states
...
class Grieves2016(Environment):
    ...
    def __init__(self, params, seg_len):
        ...
        self.n_states, self.map_states = self.get_n_states(seg_len)
        super().__init__(params, seg_len, seg_len)
    ...
    @staticmethod
    def get_n_states(seg_len):
        map_states = 7 * seg_len + 8 + 4 + 1
        return 8 * map_states, map_states
...
class Sun2020(Environment):
    ...
    def __init__(self, params, width):
        ...
        self.n_states = self.get_n_states(width, self.n_laps)
        super().__init__(params, width, width)
    ...
    @staticmethod
    def get_n_states(width, n_laps):
        return n_laps * (2 * width + 2 * (width - 2))
